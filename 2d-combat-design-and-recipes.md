...
**Status (2026-09-20, `combat` @ `a1f8e1bc`):** M0 and the playable keyboard/AI melee core (M1a–M1c) are implemented. The current UI still uses a Schwertkampf checkbox and the internal `sword` ID, with reach and optional directional swoosh controls; damage/cooldown are fixed by its preset in the editor. The generic Nahkampfangriff trait, touch attack, optional character attack states, ranged/projectile/laser delivery and published recipes are **not yet implemented**. Sections describing those features are design targets, not working instructions. The current branch must be reinspected before relying on this status.
...
The **Nahkampfangriff** trait should immediately expose damage, hit reach, cooldown and optional swoosh direction/size; show optional presentation settings separately from gameplay geometry. Additional presets such as „Schwert“ or „Kralle“ may prefill these settings, but must not rename the underlying attack type. Show only implemented options; advanced delivery and effects remain future work.
...
**Already implemented:** Optional `actor.attacks` / `baddie.attacks` arrays hold the current melee definitions, validated by the shared combat system. An absent array means no new attacks. The current `sword` ID/preset is **legacy data to preserve**, not a requirement to call future attacks swords. **Proposed:** The new Nahkampfangriff and Fernkampfangriff editor traits should read and configure these same attack definitions through one explicit adapter/mapping, rather than introduce independent damage engines or silently duplicate attacks. Specify precedence when old `sword` data and new editor trait fields coexist; do not automatically rewrite saved games when loading. The following laser example is **illustrative and not executable**.
...
The studio should offer **Nahkampfangriff** as a generic trait for actor and baddie, and **Fernkampfangriff** when a ranged delivery actually works. Each trait exposes the relevant attack's **Schaden**, **Reichweite**, **Cooldown** and optional appearance separately; the current Nahkampfangriff also exposes the existing swoosh direction and length. Provide short „Das musst du zeichnen“ help: current melee requires no additional artwork. Named weapons (Schwert, Faust, Kralle, Bogen, Laser) can be examples or later presets, not different base traits. An expandable **Kampf → Angriffe** area may later group multiple attack traits, but is not a prerequisite for the next patch. Hide or label unimplemented options rather than silently creating no-op attacks.

Currently **J activates the existing melee attack** while held (repeats according to its cooldown); **F remains interaction**. **K / Fernkampf is proposed, not implemented.** Support touch buttons for enabled attacks and later configurable bindings/additional slots. A laser can occupy the *Fernkampf* slot just like a bow: the input does **not** dictate delivery type. For baddies, a small readable **Angriffsverhalten** chooser maps AI decisions to available attacks: e.g. *bei freier Sicht und in Reichweite* for bow/laser; *nur vor der Figur und in Reichweite* for sword; *in der Nähe* for radial pulse. If multiple attacks qualify, use an explicit student-visible priority or per-attack priority/cooldown. Don't introduce pursuit/pathfinding automatically. A baddie can use a sword, a bow and a laser simultaneously as separate configured attacks, subject to bounded priorities and cooldowns.
...
**M1a–M1c — Playable melee core implemented; milestone still incomplete.** `src/static/combat_swing.js` supports both owners, forward hit geometry, walls/closed doors, once-per-target hits, cooldown, J/AI activation, procedural optional swoosh and its direction/visual length controls. A sword-specific checkbox currently creates `sword` definitions. **Next:** introduce generic Nahkampfangriff trait and editor damage/reach/cooldown/visual controls while preserving existing JSON. **Still pending:** touch activation, optional actor/baddie attack state, contextual help and a tested/published child-facing recipe. For death presentation, existing `dead` states may contain artwork or a transparent frame; do not add automatic corpse removal as a combat requirement.
...
**Settled:** J activates melee in facing direction and can repeat at cooldown; F remains interaction. Melee and ranged should appear as **generic editor traits**, not separate systems per weapon. A shared combat runtime serves actor and baddie, while optional visual swooshes never determine damage or reach. Dead baddies use the existing `dead` state, including transparent artwork when children want them to disappear. **Open:** K / ranged controls, touch buttons, attack-state/frame binding, AI behaviour for several equipped attacks, broad collision/occlusion semantics, documentation serving, and the precedence/round-trip rules for legacy `sword` JSON and newly introduced trait fields. Tune defaults only after testing gameplay.

**Next action:** Implement the generic **Nahkampfangriff** editor trait in one focused patch for both actor and baddie. Preserve current `sword` definitions and unrelated stored JSON; surface damage, reach, cooldown and optional swoosh controls. Update this status only after the user has tested and committed the actual implementation. Do not modify GitHub on their behalf.
